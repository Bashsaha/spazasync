...
| 2026-05-22 | Claude (Legal pages) | Privacy Policy + Terms of Service pages | Added `/legal/privacy` + `/legal/terms` (English-only, public). New regulatory references introduced (outside the journey/fund surface, in `.tsx` not i18n so not caught by the completeness test): **POPIA (Act 4 of 2013)** and the **Information Regulator (South Africa)** — `inforegulator.org.za`, complaints `POPIAComplaints@inforegulator.org.za`. Verify the Information Regulator URL + complaints email annually with this doc. Not added to Section A (kept scoped to journey/fund URLs + avoids WAF-flaky checker pings). | No (not a phase) |
| 2026-06-14 | Claude (BUG-055 doc cleanup) | Official-document content audit — Compliance Report, Inspection/Evidence Pack, Fund Application Pack, Trading-Permit affidavit templates, Food Safety + CoA journey copy | Removed Movestock-internal content from government-facing artefacts: stripped the internal /100 compliance score from all three PDFs; made the Landlord Affidavit + Goods Declaration fully blank templates (removed "[update in Movestock Settings]" placeholders); removed the no-storage-violating "Upload your certificate" stub from the Food Safety step. Added a generic Google-Maps search link to the CoA "where to go" fallback (no new fixed URL/fact — query is built from the owner's area at runtime). **No new external regulatory facts asserted**, so no inventory rows added; this was content-removal + a runtime-built link only. | No (not a phase) |
