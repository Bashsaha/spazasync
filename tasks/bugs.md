...
## BUG-033: Chrome "deceptive site / password entered" warning on add-teller form
**Symptom:** When the owner submits the New Teller form, Chrome flashes a "you just entered a password on a deceptive site" warning and turns the address bar grey with an X.
**Root cause:** The password field had `type="password"` with no `autocomplete` attribute. Chrome's password manager + Safe Browsing heuristics treat a password input with no `autocomplete` hint as a potential login attempt; if the form posts to a domain that isn't the user's known login site (or if the user just signed in elsewhere with a different password), Chrome flags it as deceptive.
**Fix:** Added `autoComplete="new-password"` to the password field and `autoComplete="off"` to the name field and form. `new-password` is Chrome's documented signal for "this is account creation — do not match against saved credentials, do not treat as login".
**Prevention rule:** Every password field that **creates** a credential (signup, change-password, add-teller, add-user) MUST have `autoComplete="new-password"`. Every password field that **signs into** an existing account MUST have `autoComplete="current-password"`. Never leave the attribute off — Chrome's fallback heuristic triggers Safe Browsing warnings in production.
