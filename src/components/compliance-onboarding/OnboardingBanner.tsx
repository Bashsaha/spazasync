...
export function OnboardingBanner({ onStart }: OnboardingBannerProps) {
  ...
  return (
    <div className="bg-brand-light border border-brand-light rounded-2xl p-4 mb-4">
      <p className="font-semibold text-brand-hover">{t('banner_title')}</p>
      <p className="text-sm text-brand-hover mt-1">{t('banner_subtitle')}</p>
      <div className="flex gap-2 mt-3">
        <button
          type="button"
          onClick={onStart}
          className="flex-1 bg-brand text-white font-semibold py-2 rounded-full active:bg-brand-hover text-sm"
        >
          {t('banner_btn_start')}
        </button>
        <button
